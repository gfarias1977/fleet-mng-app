...
import type { PaginatedResult, SensorRow, SensorSortField } from '@/data/sensors';
import { SensorFormDialog } from './SensorFormDialog';
import { DeleteSensorDialog } from './DeleteSensorDialog';

interface Props {
  initialData: PaginatedResult<SensorRow>;
  sensorTypes: { id: number; name: string }[];
}

type SortDir = 'asc' | 'desc';

export function SensorsClient({ initialData, sensorTypes }: Props) {
  ...
  const currentSort = (searchParams.get('sort') as SensorSortField) ?? 'name';
  const currentOrder = (searchParams.get('order') as SortDir) ?? 'asc';

  function handleSort(field: SensorSortField) {
  ...
  function SortIcon({ field }: { field: SensorSortField }) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <Input
          ...
          className="max-w-xs"
        />
        <Button onClick={() => setFormDialog({ open: true, sensor: null })}>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>
                <SortableHeader field="sensorTypeName">Tipo</SortableHeader>
              </TableHead>
              <TableHead>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button
                        ...
                        size="icon"
                        title="Editar"
                        onClick={() => setFormDialog({ open: true, sensor })}
                      >
                      ...
                      <Button
                        ...
                        onClick={() => setDeleteDialog({ open: true, sensor })}
                      >
      ...
      <SensorFormDialog
        ...
        sensor={formDialog.sensor}
        sensorTypes={sensorTypes}
        onClose={() => setFormDialog({ open: false, sensor: null })}
        onSuccess={() => {
          setFormDialog({ open: false, sensor: null });
      ...
      <DeleteSensorDialog
        ...
        sensor={deleteDialog.sensor}
        onClose={() => setDeleteDialog({ open: false, sensor: null })}
        onSuccess={() => {
